...
std::vector<TH1D*> hLowP2;
std::vector<TH1D*> hHigP2;
...
void myLoadData(std::vector<std::string> runs, const std::string& inFolder, const std::string& baseName, const std::string& psName)
{
  for(unsigned int iRun = 0; iRun < nRuns; ++iRun)
  {
    ...
    std::cout << Form("%s/%s_run%s.root",inFolder.c_str(),baseName.c_str(),runsVec[iRun].c_str()) << std::endl;
    
    if( !isTOFHIR )
      {
	TGraph* g_temp;	
	g_temp = (TGraph*)( inFile->Get(Form("%s",psName.c_str())) );
	double x,y;
	for(int iPoint = 0; iPoint < g_temp->GetN(); ++iPoint)
	  {
	    g_temp -> GetPoint(iPoint,x,y);
	    g_data[iRun] -> SetPoint(iPoint,x-40.,y);
	    g_data[iRun] -> SetPointError(iPoint,0.,0.002);
	  }
	
      }
    else
      {
	TGraph* g_temp = (TGraphErrors*)( inFile->Get(Form("%s_Vov%.2f",psName.c_str(),sipmPars.OV)) );
	g_data[iRun] = new TGraphErrors();
	double x,y, ey;
	for(int iPoint = 0; iPoint < int(0.5*g_temp->GetN()); ++iPoint)
	  {
	    g_temp -> GetPoint(iPoint,x,y);
	    if( y > sipmPars.fitYMax ) continue;
	    g_data[iRun] -> SetPoint(g_data[iRun]->GetN(),x+1.,y);
	    g_data[iRun] -> SetPointError(g_data[iRun]->GetN()-1,0.,0.2);
	  }
	for(int iPoint = 0; iPoint < int(0.5*g_temp->GetN()); ++iPoint)
	  {
	    g_temp -> GetPoint(g_temp->GetN()-iPoint-1,x,y);
	    if( y > sipmPars.fitYMax ) continue;
	    g_data[iRun] -> SetPoint(g_data[iRun]->GetN(),x+1.,y);
	    g_data[iRun] -> SetPointError(g_data[iRun]->GetN()-1,0.,0.2);
	  }
...
void myInSignal(const int& iRun, double* par = NULL)
{
  ...
  if( par )
    {
      if( parIndex[0+7*iRun] >= 0 ) amp = par[parIndex[0+7*iRun]];
      if( parIndex[1+7*iRun] >= 0 )  t0 = par[parIndex[1+7*iRun]];
      if( parIndex[2+7*iRun] >= 0 )  Rq = par[parIndex[2+7*iRun]];
      if( parIndex[3+7*iRun] >= 0 )  Cd = par[parIndex[3+7*iRun]];
      if( parIndex[4+7*iRun] >= 0 )  BW = par[parIndex[4+7*iRun]];
      if( parIndex[5+7*iRun] >= 0 ) BW2 = par[parIndex[5+7*iRun]];
      if( parIndex[6+7*iRun] >= 0 ) BW3 = par[parIndex[6+7*iRun]];
  ...
  fitPars[0+7*iRun] = amp;
  fitPars[1+7*iRun] = t0;
  fitPars[2+7*iRun] = Rq;
  fitPars[3+7*iRun] = Cd;
  fitPars[4+7*iRun] = BW;
  fitPars[5+7*iRun] = BW2;
  fitPars[6+7*iRun] = BW3;
  ...
  double tau2 = 0.133;
  double GNpe = ((*sipmPars).Cq+(*sipmPars).Cd)*((*sipmPars).OV+0.25)/(1.602*1e-19)*(*sipmPars).Npe;
  //double BW2fix = 10.275 + 8.92558e-09*GNpe; 
  double BW2fix = GNpe < 4.06809e+08 ? 0. : 6.50098e-09*(GNpe-4.06809e+08);
  double tau3 = 1./(BW2fix*3.14159*2.)*1e3; // nanosenconds (BW is in MHz)
  
  //double tauE = 0.5736; // from Elmore
  double tauE = 1./(BW3*3.14159*2.)*1e3; // from fit
  
  double tauHP2 = 1./(30.*3.14159*2.)*1e3; // additional HP with 30 MHz pole -- effective in 2C 30 MHz
  ...
  { 
    ...
    hSiPM[iRun] -> SetBinContent(ipt+1, 0.5*((*sipmPars).RF)*SiPMPulseShape(tt,*sipmPars,1.,t0)); // 0.5*RF is the total transimpedance
    //hSiPM[iRun] -> SetBinContent(ipt+1, 5.*0.5*((*sipmPars).RF)*SiPMPulseShape(tt,*sipmPars,amp,t0)); // 0.5*RF is the total transimpedance, 5x for the high gain output
    ...
    // TOFHIR bandwidth
    hLowP[iRun] -> SetBinContent(ipt+1, 1./freq*(1.-0.5/(freq*tau1))*funcLP(tt,tau1) );
    hDisc[iRun] -> SetBinContent(ipt+1, 1./freq*(1.-0.5/(freq*tau2))*funcLP(tt,tau2) );
    if( ipt == 0 )
      {
	hHigP[iRun] -> SetBinContent(ipt+1, 1.-1./freq*(1.-0.5/(freq*tau3))*funcLP(tt,tau3) );
	hHigP2[iRun] -> SetBinContent(ipt+1, 1.-1./freq*(1.-0.5/(freq*tauHP2))*funcLP(tt,tauHP2) );
	hDelP[iRun] -> SetBinContent(ipt+1, 1.-1./freq*(1.-0.5/(freq*tauE))*funcLP(tt,tauE) );
	hNorm[iRun] -> SetBinContent(ipt+1,1.725e06*amp);
      }
    else
      {
	hHigP[iRun] -> SetBinContent(ipt+1, 0.-1./freq*(1.-0.5/(freq*tau3))*funcLP(tt,tau3) );
	hHigP2[iRun] -> SetBinContent(ipt+1, 0.-1./freq*(1.-0.5/(freq*tauHP2))*funcLP(tt,tauHP2) );
	hDelP[iRun] -> SetBinContent(ipt+1, 0.-1./freq*(1.-0.5/(freq*tauE))*funcLP(tt,tauE) );
	hNorm[iRun] -> SetBinContent(ipt+1,0.);
      }
  }
...
void myDrawFit(const std::string& plotFolder, const std::string& commonLabel)
{
  ...
  for(int iRun = 0; iRun < nRuns; ++iRun)
  {
    ...
    // Draw
    TCanvas* c1;
    if( twoPanels ) c1 = new TCanvas(Form("c%d",iRun), Form("c%d",iRun), 1800., 750.);
    else            c1 = new TCanvas(Form("c%d",iRun), Form("c%d",iRun), 1050., 900.);
    if( twoPanels ) c1->Divide(2,1);
    c1 -> cd(1);
    ...
    if( !isTOFHIR ) hFrame -> GetYaxis()->SetTitle("V_{L} [V]"); 
    ...
    TLegend* legend = new TLegend(0.23,0.79,0.60,0.89);
    ...
    TLatex* latex = new TLatex(0.70,0.80,Form("V_{OV} = %.1f V", sipmPars.OV));
    latex -> SetNDC();
    ...
    TLatex* latex2 = new TLatex(0.70,0.75,Form("N_{pe} = %d", int(sipmPars.Npe)));
    latex2 -> SetNDC();
...
void fcn(int& npar, double* gin, double& f, double* par, int iflag)
{
  ...
  g_chisq -> SetPoint(g_chisq->GetN(),par[8],chisq);
  f = chisq;
}
...
void myFitPulse(const bool& minimize,
                const int& nPars_amp, const int& nPars_t0, const int& nPars_Rq, const int& nPars_Cd, const int& nPars_BW, const int& nPars_BW2, const int& nPars_BW3)
{
  int nPars = nPars_amp+nPars_t0+nPars_Rq+nPars_Cd+nPars_BW+nPars_BW2+nPars_BW3;
  ...
  for(int iPar = 0; iPar < nPars_amp; ++iPar) fitter -> SetParameter(iPar, Form("Amp"), amp0, 0.1, 0.1,10.);
  for(int iPar = 0; iPar < nPars_t0; ++iPar)  fitter -> SetParameter(nPars_amp+iPar, Form("t0"), t00, 0.05, 19., 21.);
  for(int iPar = 0; iPar < nPars_Rq; ++iPar)  fitter -> SetParameter(nPars_amp+nPars_t0+iPar, Form("Rq"), 450e3, 10e3, 0. ,1000e3);
  for(int iPar = 0; iPar < nPars_Cd; ++iPar)  fitter -> SetParameter(nPars_amp+nPars_t0+nPars_Rq+iPar, Form("Cd"), 12.4e-15, 0.2e-15, 1e-15 ,80e-15);
  ...
  for(int iPar = 0; iPar < nPars_BW3;++iPar)  fitter -> SetParameter(nPars_amp+nPars_t0+nPars_Rq+nPars_Cd+nPars_BW+nPars_BW2+iPar, "BW3", BW30, 5., 50., 200.);
...
int main(int argc, char** argv)
{
  ...
  BW30 = opts.GetOpt<float>("Input.BW30");
  ...
  int nPars_BW2 = -1;
  int nPars_BW3 = -1;
  GetFitParsFromCfg(argv[1], nRuns, parIndex, nPars_amp, nPars_t0, nPars_Rq, nPars_Cd, nPars_BW, nPars_BW2, nPars_BW3);
  ...
  for(unsigned int iRun = 0; iRun < nRuns; ++iRun)
    ...
    hLYSO.push_back(  new TH1D(Form("hLYSO_%d",iRun), "",npt,0,tmax) ); // LYSO response
    ...
    hLowP2.push_back( new TH1D(Form("hLowP2_%d",iRun),"",npt,0,tmax) ); // readout board bandwidth
    hHigP2.push_back( new TH1D(Form("hHigP2_%d",iRun),"",npt,0,tmax) ); // readout board bandwidth
    hDelP.push_back(  new TH1D(Form("hDelP_%d",iRun), "",npt,0,tmax) ); // readout board bandwidth
    hNorm.push_back(  new TH1D(Form("hNorm_%d",iRun), "",npt,0,tmax) ); // readout board bandwidth
  ...
  for(unsigned int iRun = 0; iRun < nRuns; ++iRun)
  {
    ...
    { 
      ...
      hLYSO[iRun] -> SetBinContent(ipt+1, funcScint(tt,tau_r,tau_d)); 
      //hLYSO[iRun] -> SetBinContent(ipt+1, funcLP(tt,tau_d));
    }
    hLaser[iRun] -> Scale((*sipmPars).Npe/hLaser[iRun]->Integral());
    hLYSO[iRun] -> Scale((*sipmPars).Npe/hLYSO[iRun]->Integral());
  }
  ...
  myFitPulse(bool(doMinimization),
             nPars_amp, nPars_t0, nPars_Rq, nPars_Cd, nPars_BW, nPars_BW2, nPars_BW3);
  
  myDrawFit(plotFolder,commonLabel); // draw the fit result
  
  TFile* outfile = TFile::Open("chisq.root","RECREATE");
  g_chisq->Write("g_chisq");
  outfile->Close();
  return 0;
}
